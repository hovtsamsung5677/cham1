...
def color_flood_expand(
    ...
    Сравнивает пиксели границы с их соседями по RGB расстоянию.
    """
...
def postprocess_mask(
    ...
    2. Морфологическое закрытие — заполнить внутренние разрывы от света (по умолчанию выключено)
    ...
    Args:
        ...
        apply_closing: Применять MORPH_CLOSE (по умолчанию False чтобы не соединять объекты)
...
def analyze_boundary(
    ...
    if not contact.any():
        # Нет контакта - это отдельный объект, НЕ мерджим
        return {
            "can_merge": False,
            "reason": "no_contact",
...
def multi_step_segment(
    ...
    if point_inside:
        # При клике внутри маски не используем multimask - просто оставляем как есть
        # Если нужно расширение - кликаем на границе объекта
        result = existing_mask_u8
        info["action"] = "point_inside_no_change"
        info["new_pixels"] = 0
    else:
        if not overlap.any():
...
def segment_image(
    ...
    min_component_area: int = 500,
    dilate_kernel: int = 0,
    expand_color_threshold: float = 15.0
