...
async def segment_proxy_endpoint(
    ...
    min_component_area: int = Form(
        500, description="Минимальная площадь компонента (пиксели) для сохранения в маске"),
    color_threshold: float = Form(
        60.0, description="Порог цветового расстояния для слияния границ"),
    dilate_kernel: int = Form(
        0, description="Размер ядра дилатации для постобработки"),
    ...
    try:
        ...
        data = {
            ...
            "existing_mask_counts": existing_mask_counts,
            "existing_mask_size": existing_mask_size,
        }
...
async def segment_endpoint(
    ...
    expand_color_threshold: float = Form(
        15.0, description="Порог цветового расстояния для захвата бликов")
):
    """
