...
Оптимизированная версия с torch.compile, TensorFloat32, attention slicing.
"""
import logging
import time
import traceback
...
#            logger.info("🔥 UNet compiled with torch.compile")
        ...
        _pipe.enable_attention_slicing()          # экономия памяти и ускорение
...
async def ai_recolor(
    ...
    try:
        ...
        rgb_hex = color_hex_int & 0xFFFFFF  # отсекаем альфа-канал
        ...
        # 7. Инференс (оптимизированное количество шагов)
        gen_start = time.time()
        ...
        result = _pipe(
            ...
            guidance_scale=7.5,
            num_inference_steps=12,   # <-- уменьшено с 20 для ускорения
