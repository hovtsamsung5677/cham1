...
async def get_mask(
    ...
    try:
        ...
        # Resize для SAM-2 (максимум 1024)
        max_size = 1024
        if orig_w > max_size or orig_h > max_size:
            # Сохраняем коэффициент масштабирования для корректировки координат
        ...
        # Парсим отрицательные точки (тоже масштабируем, если есть)
        neg_xs = []
        neg_ys = []
        if negative_point_x.strip() and negative_point_y.strip():
            ...
            # Масштабируем отрицательные точки
        ...
        # Сохраняем маску как PNG (0/255)
        mask_img = Image.fromarray(mask_binary * 255, mode='L')
        buf = BytesIO()
        mask_img.save(buf, format="PNG")
        logger.info(f"   Returning PNG mask, size={buf.tell()} bytes")
...
async def ai_recolor(
    ...
    try:
        ...
        # Отрицательные точки с масштабированием
        neg_xs = []
        neg_ys = []
        if negative_point_x.strip() and negative_point_y.strip():
            neg_xs = [float(x.strip()) for x in negative_point_x.split(',')]
        ...
        result = recolor_adaptive(
            image_pil, mask_pil, target_r, target_g, target_b, strength)
